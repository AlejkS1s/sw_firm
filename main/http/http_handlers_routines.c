...
esp_err_t post_routines(httpd_req_t *req) {
    ...
    entry.hour       = (uint8_t)json_get_int(root, "h", 0);
    entry.minute     = (uint8_t)json_get_int(root, "m", 0);
    ...
    if (type == RT_CIRCULATE && (entry.interval_on == 0 || entry.interval_off == 0))
        return send_error(req, E_INVALID_ARG, "circulate intervals must be non-zero");
    ...
    if (!h)
        return send_error(req, E_INTERNAL, "routine queue full or storage full");

    char buf[RESP_BUF_SIZE];
    snprintf(buf, sizeof(buf), "{\"ok\":true,\"i\":%d}", routine_index(h));
    return send_json(req, buf);
}
